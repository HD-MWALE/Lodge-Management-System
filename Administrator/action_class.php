...
Class Action {
	...
	function login(){
		extract($_POST);
		$qry = $this->db->query("SELECT *,concat(firstname,' ',lastname) as fullname FROM staff WHERE email = '".$email."' and password = '".md5($password)."' ");
		if($qry->num_rows > 0){
		...
		/*
		$sql = "SELECT `staff_id`, `firstname`, `lastname`, `contact`, `username`, `password`, `avatar`, `last_login`, `position`, `date_added`, `date_updated`, concat(firstname,' ',lastname) as fullname FROM staff where email = ?";
		...
		if($stmt->execute()){
			$stmt->store_result();
			if($stmt->num_rows == 1){      
				$stmt->bind_result($staff_id, $firstname, $lastname, $contact, $username, $hashed_password, $avatar, $last_login, $position, $date_added, $date_updated, $fullname);
				$stmt->fetch();
	...
	function save_customer(){
		...
		if(isset($customer_id)){
			$r = $this->update_customer($customer_id);
			return $r;
		}else{
			$sql = "SELECT `customer_id` FROM `customer` WHERE `email` = ?";
			$stmt = $this->db->prepare($sql);
			$stmt->bind_param("i", $param_email);
			$param_email = $email;
			$stmt->execute();
			$stmt->store_result();
	...
	function delete_staff(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM staff where staff_id = ".$id);
		if($delete)
			return 1;
	}
